...
<?php
include 'connect.php';
$pid=$_GET['pid'];$sel="SELECT * FROM `bustransactions` WHERE `T_No.` =$pid";
...
if($_SESSION['tickets']==0){
  ...
  $sql = "DELETE FROM pricebus WHERE `source` LIKE '$source' AND `dest` LIKE '$dest'AND `Date` = '$date' AND Class='$class'";
...
if (mysqli_query($connect, $sql)) {
  ...
  $sql = "UPDATE pricebus SET number='$tickets',Amt='$Amt'  WHERE `source` LIKE '$source' AND `dest` LIKE '$dest'AND `Date` = '$date' AND class='$class' ";
