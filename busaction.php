...
$final=0;
$Route="";
$price="SELECT * FROM `pricebus` WHERE `source` LIKE '$source' AND `dest` LIKE '$dest' AND `Date`='$date' AND `class`='$class' AND `no`>='$no'";
$result = $connect->query($price);
while ($row = $result->fetch_assoc()) {
  $final = ($row["Amt"]/$row['no']) *$no ;
  $tickets=$row['no']-$no;
  $da = $row["Date"] ;
}
if ($source == $dest)
...
echo'<center>
<td><form action="pay1.php"><button style="background-color: black; padding: 25px 70px; type="submit"; align=center;"><span style=color:white;><h3>Checkout</h3></span></a></td></button></form>
';
...
if ($source == $dest) {
    ...
    echo "<h2><center><b>No Available Tickets. Please try again later.</b></center><h2>";
} else {
    echo "<h1><center><b><img src=https://img.icons8.com/cotton/80/000000/route.png>SELECT ROUTE AND PROCEED TO CHECKOUT :</center></b></h1>";
    echo "<h1><center>$source to $dest</center></h1>";
